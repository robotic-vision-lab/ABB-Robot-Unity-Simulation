...
public class basicRobotController : MonoBehaviour {
    ...
    public UnityEngine.Quaternion ef_r;
    ...
    float GetGradient(float t1, float t2, float t3, float t4, float t5, float t6, int joint_num){
        float D = UnityEngine.Vector3.Distance(GetFK(t1, t2, t3, t4, t5, t6).GetColumn(3), target_wc);
        if (joint_num == 1){
            t1 = t1 + delta;
        }
        else if (joint_num == 2){
        ...
        float D_prime = UnityEngine.Vector3.Distance(GetFK(t1, t2, t3, t4, t5, t6).GetColumn(3), target_wc);
        return((D_prime - D) / delta);
    }

    void GetIK(float t1, float t2, float t3, float t4, float t5, float t6){
        for (int i = 0; i < iterations; i++){
            float error_distance = UnityEngine.Vector3.Distance(GetFK(t1, t2, t3, t4, t5, t6).GetColumn(3), target_wc);
    ...
    void FixedUpdate(){
        ...
        target_dist = UnityEngine.Vector3.Distance(ef_wc, target_wc);
